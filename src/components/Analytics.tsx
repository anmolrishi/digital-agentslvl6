...
export default function Analytics({ userId }: AnalyticsProps) {
  ...
  const currentModeAnalytics = analytics[selectedMode] || {};
  const totalCalls = Object.keys(currentModeAnalytics).length;
  const totalDuration = Object.values(currentModeAnalytics).reduce(
    (sum, call) => sum + call.duration_ms,
    0,
  );
  const averageDuration =
    totalCalls > 0 ? totalDuration / totalCalls / 1000 : 0;

  const callDurationData = Object.entries(currentModeAnalytics).map(
    ([callId, data]) => ({
  ...
  const sentimentData = Object.values(currentModeAnalytics).reduce(
    (acc, call) => {
  ...
  const handleRowClick = (callId: string) => {
    setSelectedCall({ callId, data: currentModeAnalytics[callId] });
    onOpen();
  };
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div
        className={`flex-grow p-8 transition-all duration-300 ${isNavbarExpanded && !userId ? "ml-64" : "ml-20"}`}
      >
        <VStack spacing={8} align="stretch">
          <div className="flex items-center justify-between mb-8">
            <Heading as="h1" size="xl" color="blue.800">
              {restaurantName}'s Analytics
            </Heading>
          ...
          <Box bg="white" p={6} borderRadius="md" boxShadow="lg">
            ...
            <TableContainer>
              <Table variant="simple">
                ...
                <Tbody>
                  {Object.entries(currentModeAnalytics).map(
                    ([callId, data]) => (
                      <Tr
                        key={callId}
                      ...
                      >
                        ...
                        <Td>
                          {data.call_analysis.user_sentiment || "Unknown"}
                        </Td>
            ...
            <ModalBody>
              <VStack align="start" spacing={4}>
                ...
                <Text>
                  ...
                  {selectedCall.data.call_analysis
                    .agent_task_completion_rating || "N/A"}
                </Text>
                <Text>
                  <strong>Call Successful:</strong>{" "}
                  {selectedCall.data.call_analysis.call_successful
                    ? "Yes"
                    : "No"}
                </Text>
                <Text>
