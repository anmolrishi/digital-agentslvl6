...
export function generatePrompt(userData: any, mode: Mode): string {
  ...
  const basePrompt = `You are an AI assistant for a company named {{restaurantName}}. Your name is {{botName}}. You should maintain a {{tone}} tone throughout the conversation.

The details of the company are:

Company Name: {{restaurantName}}
Team Size: {{seatingCapacity}}
Location: {{address}}
...
{{menu}}`;
  ...
  const modePrompts = {
    customer: `Your role is to assist customers with inquiries about our company, handle service requests, and provide information about our offerings and services.`,
    operations: `Your role is to assist with internal operations, including team management, resource allocation, and workflow coordination.`,
    sales: `Your role is to handle business development inquiries, partnership opportunities, and potential client discussions.`
  };
